...
contains
    ...
    module procedure output_results
        ...
        real(rk) :: min_val, max_val, mean_val
        integer :: ierr       
        !=========================================================================
        ...
        mean_val = sum(this%con) / real(total_points, rk)    ! Mean concentration
        ...
        print *, '  |                                                                      |'
		print '(A, I0, A, I0, A)', &
    '   |    Concentration Sample (first ', SAMPLE_SIZE, ' x ', SAMPLE_SIZE, '):                               |'
        print *, '  |                                                                      |'
        ...
        ! Precision type: reflects the actual working precision (rk), not
        ! a hardcoded assumption. Prevents readers from misinterpreting
        ! double-precision data as 32-bit floats (or vice versa).
        if (rk == dp) then
            write(fileunit, iostat=ierr) PRECISION_DOUBLE
        else
		...
		print '(A, F0.2, A)', &
			'   |    File size      : ', &
			real(32 + total_points * (storage_size(this%con) / 8), rk) / 1024.0_rk, &
			' KB                                        |'
        print '(A, A, A)', &
