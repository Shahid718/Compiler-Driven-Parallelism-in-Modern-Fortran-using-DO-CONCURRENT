...
module phase_field_module
    ...
    type, public :: PhaseFieldGrid
        integer :: Nx, Ny
        real(rk) :: dx, dy, A, dt, mobility, grad_coef, c0
        real(rk), dimension(:,:), allocatable :: con
        real(rk), dimension(:,:), allocatable :: r
        real(rk), dimension(:,:), allocatable :: dfdcon
        real(rk), dimension(:,:), allocatable :: lap_con
        real(rk), dimension(:,:), allocatable :: dummy_con
        real(rk), dimension(:,:), allocatable :: lap_dummy
        real(rk), dimension(:,:), allocatable :: con_next
    contains
    ...
    interface
        ...
        !    seed  - RNG seed for reproducible runs (intent: in, optional)
        !            0 or absent => non-deterministic (OS entropy)
        !-----------------------------------------------------------------------			
        module subroutine init_microstructure(this, c0, noise, seed)
            class(PhaseFieldGrid), intent(inout) :: this
            real(rk), intent(in) :: c0, noise
            integer(i_sp), intent(in), optional :: seed
        end subroutine
        ...
        module subroutine time_integration(this, dt, mobility, grad_coef)
            class(PhaseFieldGrid), intent(inout) :: this
            real(rk), intent(in) :: dt, mobility, grad_coef
        end subroutine
        ! swap_fields : Exchange two allocatable arrays
        !-----------------------------------------------------------------------
        !  Arguments :
        !    this  - First allocatable array (intent: inout)
        !    that  - Second allocatable array (intent: inout)
        !-----------------------------------------------------------------------
